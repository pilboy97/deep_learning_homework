...
class Conv2D:
    def __init__(self, input_channels, output_channels, filter_size, stride=1, padding=1):  # padding 기본값 변경
        ...
        self.b = np.zeros(output_channels)  # 편향을 출력 채널만큼 초기화
    ...
    def forward(self, X):
        ...
        # X: Input data, shape (batch_size, input_channels, height, width)
        ...
        # Reshape the weights correctly
        W_reshaped = self.W.reshape(self.output_channels, -1).T  # Shape: (input_channels * kernel_height * kernel_width, output_channels)
    ...
    def backward(self, d_out):
        ...
        db = np.sum(d_out, axis=(0, 2, 3))  # 편향 기울기
